// NoIntervention.cpp : Defines the entry point for the console application.
//
	 ...
	 //generate a seed for each replication
		...
		{//simulating years
		 //for each year the birth rate is calculated for new people who are added to the age-group 20-25.
			...
			delay_results.open("C:/Users/snasrol/Google Drive/Research-Diabeties simulation/Code, Data, Results/Me/Results/3rd round/NoIntervention/30years/NoIntervention-delays.txt", std::ofstream::out | std::ofstream::app);
			...
			{//for each person alive
			 //updates the bmi value
				...
				if (v_people_alive.at(ap_ind).person_age >= 25 && v_people_alive.at(ap_ind).person_age <= 65 && v_people_alive.at(ap_ind).bmi_value >= 25 && v_people_alive.at(ap_ind).diabetes_status == "Predia")
				{//inclusion criteria for people who go under intervention
				no_intervention++;
				//DPP(RG) intervention causes the bmi value to drop 7%
				v_people_alive.at(ap_ind).bmi_value -= 0.07*v_people_alive.at(ap_ind).bmi_value;
				}
				} */
			...
			std::ofstream results;
			results.open("C:/Users/snasrol/Google Drive/Research-Diabeties simulation/Code, Data, Results/Me/Results/3rd round/NoIntervention/30years/NoIntervention-statistics.txt", std::ofstream::out | std::ofstream::app);
			...
			results0.open("C:/Users/snasrol/Google Drive/Research-Diabeties simulation/Code, Data, Results/Me/Results/3rd round/NoIntervention/30years/NoIntervention-statistics0.txt", std::ofstream::out | std::ofstream::app);
			results0 << year << '\t' << no_diabetes_by_age[0] << '\t' << no_diabetes_death_by_age[0] << std::endl;
			results0.close();
			std::ofstream results1;
			results1.open("C:/Users/snasrol/Google Drive/Research-Diabeties simulation/Code, Data, Results/Me/Results/3rd round/NoIntervention/30years/NoIntervention-statistics1.txt", std::ofstream::out | std::ofstream::app);
			results1 << year << '\t' << no_diabetes_by_age[1] << '\t' << no_diabetes_death_by_age[1] << std::endl;
			results1.close();
			std::ofstream results2;
			results2.open("C:/Users/snasrol/Google Drive/Research-Diabeties simulation/Code, Data, Results/Me/Results/3rd round/NoIntervention/30years/NoIntervention-statistics2.txt", std::ofstream::out | std::ofstream::app);
			results2 << year << '\t' << no_diabetes_by_age[2] << '\t' << no_diabetes_death_by_age[2] << std::endl;
			results2.close();
			std::ofstream results3;
			results3.open("C:/Users/snasrol/Google Drive/Research-Diabeties simulation/Code, Data, Results/Me/Results/3rd round/NoIntervention/30years/NoIntervention-statistics3.txt", std::ofstream::out | std::ofstream::app);
			results3 << year << '\t' << no_diabetes_by_age[3] << '\t' << no_diabetes_death_by_age[3] << std::endl;
			results3.close();
			...
			for (int age_ind = 0; age_ind < no_age_groups; age_ind++)
			{
				detail_results[age_ind].open("C:/Users/snasrol/Google Drive/Research-Diabeties simulation/Code, Data, Results/Me/Results/3rd round/NoIntervention/30years/NoIntervention-details" + std::to_string(age_ind) + ".txt", std::ofstream::out | std::ofstream::app);
